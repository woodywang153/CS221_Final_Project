...
def forward_propagate(inputs, W_f, b_f, W_l, b_l):
	seq_len = np.array(inputs.get_shape()[2])
	print inputs.get_shape()
	lstm_cell_fw = tf.nn.rnn_cell.BasicLSTMCell(lstm_units)
	lstm_cell_fw = tf.nn.rnn_cell.DropoutWrapper(cell=lstm_cell_fw, output_keep_prob=0.75)
	lstm_cell_bw = tf.nn.rnn_cell.BasicLSTMCell(lstm_units)
	lstm_cell_bw = tf.nn.rnn_cell.DropoutWrapper(cell=lstm_cell_bw, output_keep_prob=0.75)
	_, states = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw, lstm_cell_bw, inputs, sequence_length = seq_len, dtype=tf.float32)
